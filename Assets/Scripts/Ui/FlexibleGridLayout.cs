...
{
    /// <summary>
    ///     Layout Group controller that arranges children in bars, fitting as many on a line until total size exceeds parent
    ///     bounds
    /// </summary>
    ...
    public class FlexibleGridLayout : LayoutGroup
    {
        ...
        [Tooltip(
            "Constrains the size of the children to the size they would have at an x amount of items. Leave at 0 to always expand them as much as possible.")]
        public int constrainChildrenSizeToSizeAtCount;
        ...
        {
            var childCount =
                constrainChildrenSizeToSizeAtCount == 0 || rectChildren.Count >= constrainChildrenSizeToSizeAtCount
                    ? rectChildren.Count
                    : constrainChildrenSizeToSizeAtCount;
            float iColumn;
        ...
        public float SetLayout(int axis, bool layoutInput)
        {
            ...
            for (var i = 0; i < rectChildren.Count; i++)
            {
                ...
                if (startAxis == Axis.Horizontal)
                {
                    if (invertOrder) index = IsLowerAlign ? rectChildren.Count - 1 - i : i;
                ...
                {
                    if (invertOrder) index = IsRightAlign ? rectChildren.Count - 1 - i : i;
                    child = rectChildren[index];
                ...
                if (currentBarSize + childSize > workingSize &&
                    !Mathf.Approximately(currentBarSize + childSize, workingSize))
                {
                    ...
                    if (!layoutInput)
                    {
                        if (startAxis == Axis.Horizontal)
                        {
                            ...
                            LayoutRow(_itemList, currentBarSize, currentBarSpace, workingSize, padding.left, newOffset,
                                axis);
                        }
                        else if (startAxis == Axis.Vertical)
                        {
                            var newOffset = CalculateColHorizontalOffset(groupSize, offset, currentBarSpace);
                            LayoutCol(_itemList, currentBarSpace, currentBarSize, workingSize, newOffset, padding.top,
                                axis);
                        }
                    }
                ...
                if (childOtherSize > currentBarSpace) currentBarSpace = childOtherSize;

                // Don't do this for the last one
                if (i < rectChildren.Count - 1) currentBarSize += spacingBetweenElements;
            }
            ...
            if (!layoutInput)
            {
                if (startAxis == Axis.Horizontal)
                {
                    ...
                    currentBarSize -= spacingBetweenElements;
                    LayoutRow(_itemList, currentBarSize, currentBarSpace,
                        workingSize - (ChildForceExpandWidth ? 0 : spacingBetweenElements), padding.left, newOffset,
                        axis);
                }
                else if (startAxis == Axis.Vertical)
                {
                    ...
                    currentBarSize -= spacingBetweenElements;
                    LayoutCol(_itemList, currentBarSpace, currentBarSize,
                        workingSize - (ChildForceExpandHeight ? 0 : spacingBetweenElements), newOffset, padding.top,
                        axis);
                }
            }
            ...
            if (layoutInput) SetLayoutInputForAxis(offset, offset, -1, axis);
            return offset;
        }
            ...
            float xOffset, float yOffset, int axis)
        {
            ...
            else if (ExpandHorizontalSpacing)
            {
                ...
                if (_itemList.Count > 1)
                {
                    ...
                    else if (IsRightAlign) xPos -= extraSpacing * (_itemList.Count - 1);
                }
            }
            ...
            for (var j = 0; j < _itemList.Count; j++)
            {
                ...
                else if (IsLowerAlign) yPos += rowHeight - rowChildHeight;
                ...
                if (j < _itemList.Count - 1) xPos += rowChildWidth + spacing.x;
            }
        }

        protected void LayoutCol(IList<RectTransform> contents, float colWidth, float colHeight, float maxHeight,
            float xOffset, float yOffset, int axis)
        {
            var yPos = yOffset;
            ...
            else if (ExpandHorizontalSpacing)
            {
                ...
                if (_itemList.Count > 1)
                {
                    ...
                    else if (IsLowerAlign) yPos -= extraSpacing * (_itemList.Count - 1);
                }
            }
            ...
            for (var j = 0; j < _itemList.Count; j++)
            {
                ...
                var rowChildHeight =
                    cellSize.y + extraHeight; //LayoutUtility.GetPreferredSize(rowChild, 1) + extraHeight;

                if (ChildForceExpandWidth) rowChildWidth = colWidth;
                ...
                else if (IsRightAlign) xPos += colWidth - rowChildWidth;
                ...
                if (j < _itemList.Count - 1) yPos += rowChildHeight + spacing.y;
            }
        }
